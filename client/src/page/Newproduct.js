...
const handleSubmit = async(e) => {
  ...
  if(name && image && category && price){
    const fetchData = await fetch(`${process.env.REACT_APP_SERVER_DOMIN || "http://localhost:8080"}/uploadProduct`,{
      method : "POST",
      headers : {
  ...
  return (
    <div className="p-4">
      <form className="m-auto w-full max-w-md shadow flex flex-col p-3 bg-white"  onSubmit={handleSubmit}>
        ...
        <button className="bg-red-500 hover:bg-red-500 text-white text-lg font-medium my-2 drop-shadow">Save</button>
      </form>
    </div>
  )
}
