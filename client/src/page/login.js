   ...
   if( email && password ){
    try {
      const fetchData = await fetch(
        `${process.env.REACT_APP_SERVER_DOMIN || 'http://localhost:8080'}/login`,
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    <div className="w-full max-w-sm bg-white m-auto flex flex-col p-4">
        ...
        <form className="w-full py-3 flex flex-col" onSubmit={handleSubmit}>
          ...
          <p className="text-left text-sm mt-2">Don't have account ? <Link to={"/sign up"} className="text-red-500 underline">Sign up</Link></p>
        </div>
        </div>
  )
}
